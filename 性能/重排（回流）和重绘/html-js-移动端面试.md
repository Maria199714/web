...
https://juejin.cn/post/6844904083212468238参考链接
...
3. 布局渲染树（reflow/layout）：从根节点递归调用，计算每一个元素的大小、位置等，给出每个节点所应该在屏幕上出现的精确坐标，将元素在页面上的位置算出来；
...
　　　　　　　　　　所以，**重排必定会引发重绘**，但重绘不一定会引发重排。
 ...
 　　**触发重排的条件：任何页面布局和几何属性的改变都会触发重排（位置被改变就会引发重排），比如：**
...
2、我们要注意的优化：我们要减少重绘和重排就是要减少对渲染树的操作，即不要经常的去操作DOM元素，则我们可以合并多次的DOM和样式的修改。并减少对style样式的请求。
...
这就可以排除,用其他病毒或者软件自动申请用户及自动登陆等有效防止这种问题。
...
//class 新的ES6语法 就是 构造函数的 另一种新写法 语法糖
    class Person {
        //构造器
        constructor(){
           this.name='建林'
...
//继承
    class Teacher extends Person {
        constructor(){
            super();// 继承必须写super 他就是父类 上面的那个 constructor 在this之前写
           
            this.name='思聪';//自己的会覆盖继承的
        }
...
异步 ：遇到异步就在执行异步的时候，同时往下面进行执行，不会去等待异步执行完毕。

哪些是异步：定时器、Ajax、事件处理、读取文件

#### 4.1 问题分析

​		考察 地狱回调（疯狂套娃）  Promise  async 与await

可以使用Promise来防止出现回调地狱的问题。链式编程，就没有那么多的嵌套。

Axios就是利用Promise语法对Ajax的一种封装。

#### 4. 2 核心问题讲解-有代码

`async`函数返回一个 Promise 对象，可以使用`then`方法添加回调函数。当函数执行的时候，一旦遇到`await`就会先返回，等到异步操作完成，再接着执行函数体内后面的语句。

async和await  最简单是使用 就是 可以省略掉then  简单快捷  代码看起来清晰。

遇到await会等待异步操作执行成功，才对异步下面的代码进行执行。
...
async 函数是什么？一句话，它就**是 Generator 函数的语法糖**，简化了写法。
...
装饰器是一种**函数**，写成`@ + 函数名`。它可以放在类和类方法的定义前面。
...
举例：
//在类或者类属性方法 前面写上 @函数名 就相当于要调用这个函数 这里就相当于调用了这个函数
@testable
...
凡是 异步代码和定时代码 都会等待 主程序执行完毕 再开始执行。

异步操作：定时器、延时器、Ajax、绑定的onclick事件函数、Promise

注意：new Promise 会立刻执行。

#### 7.1 问题分析

考察  js 代码的执行过程  执行顺序 而.then 是需要异步的
...
平时开发代码 那个是开发的  要是上线  需要 npm run build（打包）  这个生成的代码才是我们要的
...
2. 注意： **在公司里面 一般 你只需要** 
   ...
   ​          2 或者 你**build** 之后 把 build的代码 发给 后台   后台给你把网址放到服务器
